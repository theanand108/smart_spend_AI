...
SEMANTIC_PATTERNS: dict[str, tuple[str, ...]] = {
    ...
    "Transfer / Personal": (
        r"\bmy\s+share\b",
        r"\bmy\s+part\b",
        r"\bsplit\s+(?:the\s+)?(?:bill|dinner|food)\b",
        r"\bpay(?:ing)?\s+back\b",
        r"\bpaid\s+back\b",
        r"\bmoney\s+i\s+owed\b",
        r"\bsent\s+(?:money\s+)?(?:to|for)\s+(?:my\s+)?friend\b",
        r"\btransferred\s+(?:money\s+)?(?:to|for)\s+(?:my\s+)?friend\b",
        r"\breimbursement\b",
    ),
    "Shopping": (
        r"\bclothes?\b",
...
def semantic_note_evidence(note: Any) -> dict[str, Any]:
    ...
    if not text:
        return {"candidates": [], "confidence": 0.0, "reason": "No note provided."}

    if any(re.search(pattern, text) for pattern in NEGATION_PATTERNS):
        return {"candidates": [], "confidence": 0.0, "reason": "Note contains a negation pattern; semantic category inference is unsafe."}
    ...
    if not matches:
        return {"candidates": [], "confidence": 0.0, "reason": "No meaningful semantic category signal found."}
    ...
    if len(matches) > 1 and matches[0][1] == matches[1][1]:
        return {
            ...
            "confidence": 0.0,
            "reason": "Multiple categories have equally strong semantic evidence.",
        }
    ...
    return {
        ...
        "reason": f"Semantic note pattern matched {top[0]} evidence.",
    }
